...
def init_agent_compat(agent) -> None:
    """Version-compatible skrl agent initialization for model evaluation.

    Some skrl versions expect trainer_cfg to be a dataclass rather than a dict.
...
def _apply_observation_preprocessor(agent, obs: torch.Tensor, debug: bool = False, step: int = 0) -> torch.Tensor:
    ...
    if debug:
        print(f"[DEBUG][eval step {step}] after observation preprocessor", flush=True)
...
def _policy_forward(policy, obs: torch.Tensor, debug: bool = False, step: int = 0) -> torch.Tensor:
    ...
    for attr in ["net", "network", "actor", "model"]:
        ...
        if module is not None and callable(module):
            ...
            if isinstance(out, tuple):
                out = out[0]
            if debug:
                print(f"[DEBUG][eval step {step}] after policy.{attr}(obs)", flush=True)
            return out
    ...
    if debug:
        print(f"[DEBUG][eval step {step}] after policy.compute", flush=True)
...
def direct_policy_action(agent, states, *, debug: bool = False, step: int = 0) -> torch.Tensor:
    ...
    This intentionally does not call agent.act(), because G1/Go2 model_test
    already showed agent.act can block in the current skrl + IsaacLab setup.
    """
    if debug:
