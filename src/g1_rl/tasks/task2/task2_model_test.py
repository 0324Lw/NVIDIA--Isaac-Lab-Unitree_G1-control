...
parser = argparse.ArgumentParser(description="Evaluate Unitree G1 Task2 Omni pure-RL skrl PPO model")
...
def print_table(summary):
    print("\n" + "=" * 168)
    print("G1 Task2 Omni Pure-RL Model Test Summary")
    print("=" * 168)
...
def direct_policy_action(agent, states):
    """Deterministic safe policy inference for G1 model evaluation.

    Training still uses skrl PPO normally. Evaluation bypasses high-level
    agent.act because G1 eval already showed it can block on this skrl build.
    """
...
def apply_eval_curriculum_overrides(cfg: Task2Config) -> None:
    cfg.cmd_vx_stage0 = (0.00, 0.04)
    cfg.cmd_vy_stage0 = (0.00, 0.00)
    cfg.cmd_wz_stage0 = (0.00, 0.00)

    cfg.cmd_vx_stage1 = (-0.04, 0.16)
    cfg.cmd_vy_stage1 = (0.00, 0.00)
    cfg.cmd_wz_stage1 = (0.00, 0.00)

    cfg.cmd_smoothing_factor = 0.08
...
def main():
    ...
    apply_eval_curriculum_overrides(cfg)

    base_env = G1OmniEnv(cfg)
    ...
    stacked_env = G1FrameStackWrapper(
        base_env,
        log_dir=str(PROJECT_ROOT / "logs" / "task2_eval_tmp"),
        n_stack=5,
    ...
    print("\n" + "=" * 150)
    print("Unitree G1 Task2 Omni pure-RL skrl model test started")
    print("=" * 150)
    ...
    print("note       : pure-RL omni baseline evaluation, not HoloSoma / BeyondMimic imitation pipeline")
    print("=" * 150 + "\n")

    try:
        with tqdm(total=int(args_cli.steps), desc="G1 Task2 Omni Model Test", dynamic_ncols=True, mininterval=0.5) as pbar:
                    ...
                    pbar.set_postfix(
                        {
                            ...
                            "yawerr": f"{flat.get('telemetry/Yaw_Error', 0.0):.3f}",
        ...
        print("\n✅ G1 Task2 model test rollout finished")
        ...
        print("G1 Task2 model test checklist:")
        print("1. Smoke checkpoint 表现差是正常的，重点检查是否稳定推理、无 NaN/Inf。")
        print("2. Task2 是 pure-RL omni baseline，不代表专业人形机器人动作控制最终路线。")
        print("3. 正式训练 checkpoint 应逐步看到 Lin_Error / Yaw_Error 下降。")
        print("4. Actual_Vx/Vy/Wz 接近 Cmd_Vx/Vy/Wz 前，先确保 Fall_Rate 和 Base_Height 稳定。")
        print("5. 如果一直原地站立，说明 pure-RL 奖励存在局部最优，需要谨慎调 curriculum / command range。")
