...
parser = argparse.ArgumentParser(
    description="Train Unitree G1 Task3 whole-body pure-RL baseline with skrl PPO"
)
...
parser.add_argument(
    "--pretrained-task2",
    "--pretrained",
    dest="pretrained_task2",
    type=str,
    default=os.environ.get("G1_TASK2_PRETRAINED", ""),
    help="Optional Task2 skrl checkpoint for warm-start.",
)
parser.add_argument(
    "--resume",
    type=str,
    default="",
    help="Optional Task3 checkpoint file or checkpoint directory to resume.",
)
...
# Conservative Task3 curriculum overrides
parser.add_argument("--cmd-smoothing-factor", type=float, default=0.08)
parser.add_argument("--stage0-vx-max", type=float, default=0.05)
parser.add_argument("--stage1-vx-max", type=float, default=0.10)
parser.add_argument("--start-arm-gain-k", type=float, default=0.18)

# Logging
parser.add_argument("--log-root", type=str, default=os.environ.get("RT_G1_TASK3_LOG_ROOT", default_log_root("task3")))
...
def make_log_dir() -> str:
    ...
    if not run_name:
        run_name = f"g1_task3_whole_body_skrl_ppo_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def task3_progress_postfix(env_steps: int, start_time: float, reward_mean: float, done_count: int, info: Dict[str, Any]):
    ...
    return {
        ...
        "sync": f"{flat.get('reward_components/R_Arm_Leg_Sync', 0.0):+.3f}",
    }
...
def print_update(pbar, update_id, env_steps, total_steps, elapsed, num_envs, rollouts, info, ppo, lr):
    ...
    pbar.write(
        "\n".join(
            [
                "\n" + "=" * 124,
                f"📊 [G1 Task3 Whole-Body pure-RL skrl PPO 更新 {update_id}] "
                f"总步数: {env_steps:,} / {total_steps:,} | FPS: {stat['fps_env_steps']:,.0f} | LR: {lr:.3e}",
                "=" * 124,
...
def build_skrl_cfg(env, log_dir: str):
    ...
    cfg["experiment"].update(
        {
            "directory": log_dir,
            "experiment_name": "g1_task3_whole_body_skrl",
...
def try_load_pretrained(agent, models, pretrained_path: str, device: str) -> bool:
    if not pretrained_path:
        print("[INFO] 未指定 --pretrained-task2，将从随机初始化开始 Task3。")
        return False

    ckpt_path = _resolve_checkpoint_file(pretrained_path, default_name="g1_task2_omni_model.pt")

    if not os.path.exists(ckpt_path):
        print(f"[WARN] 找不到 Task2 预训练模型: {ckpt_path}")
        print("[WARN] 将从随机初始化开始 Task3。")
        return False

    print("\n" + "=" * 112)
    print(f"🔁 尝试加载 Task2 预训练模型: {ckpt_path}")
    print("=" * 112)

    try:
        agent.load(ckpt_path)
        print("✅ 已通过 agent.load() 成功加载 Task2 预训练模型")
    ...
    try:
        ...
        print(f"[WARN] 手动加载 Task2 预训练模型失败: {type(exc).__name__}: {exc}")

    print("[WARN] Task2 预训练模型加载失败，将从随机初始化开始 Task3。")
    return False


def try_resume_task3(agent, resume_path: str) -> int:
    if not resume_path:
        return 0

    ckpt_path = _resolve_checkpoint_file(resume_path, default_name="g1_task3_whole_body_model.pt")

    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(f"--resume 指定的 checkpoint 不存在: {ckpt_path}")

    print("\n" + "=" * 112)
    print(f"🔁 恢复 Task3 checkpoint: {ckpt_path}")
    print("=" * 112)
...
def apply_task3_conservative_overrides(env_cfg: Task3Config) -> None:
    env_cfg.cmd_vx_stage0 = (0.00, float(args_cli.stage0_vx_max))
    env_cfg.cmd_vy_stage0 = (0.00, 0.00)
    env_cfg.cmd_wz_stage0 = (0.00, 0.00)

    env_cfg.cmd_vx_stage1 = (0.04, float(args_cli.stage1_vx_max))
...
def save_train_metadata(path, env_steps, total_env_steps, num_envs, base_env, stacked_env, pretrained_path, pretrained_loaded):
    torch.save(
        {
            "stage": "unitree_g1_task3_whole_body_pure_rl_baseline",
            "algorithm": "skrl_ppo",
            ...
            "pretrained_task2": str(pretrained_path),
            ...
            "arm_joint_names": list(base_env.cfg.arm_joint_names),
            "command_curriculum": {
...
def save_agent_checkpoint(agent, save_dir: str, env_steps: int, total_env_steps: int, num_envs: int, base_env, stacked_env, pretrained_path: str, pretrained_loaded: bool):
    ...
    agent.save(os.path.join(save_dir, "g1_task3_whole_body_model.pt"))
...
def main():
    ...
    print("\n" + "=" * 124)
    print("🚀 Unitree G1 Task3: Whole-Body Locomotion Pure-RL skrl PPO 训练启动")
    print("=" * 124)
    ...
    print("[INFO] 定位说明：这是 G1 人形机器人 pure-RL whole-body baseline，不是 HoloSoma / BeyondMimic。")
    ...
    apply_task3_conservative_overrides(env_cfg)
    ...
    if args_cli.start_k > 0:
        ...
        print(
            f"[INFO] 已设置 Task3 初始课程进度 start_k={args_cli.start_k:.4f}, "
    ...
    if args_cli.resume:
        resume_env_steps = try_resume_task3(agent, args_cli.resume)
        base_env.global_steps = int(resume_env_steps)
        print(f"[INFO] 已恢复 Task3 global_env_steps={resume_env_steps:,}")
    else:
        pretrained_loaded = try_load_pretrained(agent, models, args_cli.pretrained_task2, env.device)
    ...
    print("\n🔥 [G1 Task3 Whole-Body skrl PPO 已点火]")
    print("👉 训练目标：保留 Task2 低速全向移动能力，逐步解冻上肢动作并学习全身协同。")
    print("👉 Policy/Critic 输入：123 维单帧观测 × 5 帧 = 615。")
    print("👉 动作维度：23，与 Task1/Task2 模型结构兼容。")
    print("👉 数据：g1_whole_body_walk.pt，必须包含 arm_swing_ref。")
    print("👉 当前仍然是 pure-RL baseline，不是 HoloSoma / BeyondMimic imitation pipeline。")
    print("👉 日志重点：Fall_Rate、Cmd/Actual、Arm_Action_Gain、R_Arm_Ref、R_Arm_Leg_Sync。\n")
    ...
    try:
        ...
        with tqdm(
            ...
            desc="G1 Task3 Whole-Body pure-RL skrl PPO",
            unit="steps",
            dynamic_ncols=True,
            mininterval=0.5,
            smoothing=0.05,
                ...
                if env_steps - last_save >= save_freq_env_steps:
                    ...
                    try:
                        save_agent_checkpoint(
                            ...
                            base_env=base_env,
                            stacked_env=stacked_env,
                            pretrained_path=args_cli.pretrained_task2,
                            pretrained_loaded=pretrained_loaded,
                        )
                        pbar.write(f"\n💾 [G1 Task3 备份] 总步数: {env_steps:,} | 已保存至: {save_dir}\n")
                    except Exception as exc:
        ...
        try:
            ...
            print(f"✅ G1 Task3 模型与归一化统计已保存至 {final_dir}")
        except Exception as exc:
        ...
        print("✅ G1 Task3 Whole-Body skrl PPO 训练管线安全退出")
