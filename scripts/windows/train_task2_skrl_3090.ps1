param(
    ...
    [int]$NumEnvs = 512,
    [Int64]$TotalEnvSteps = 500000000,
    [int]$Rollouts = 64,
    [int]$LearningEpochs = 5,
    [int]$MiniBatches = 8,
    [double]$Lr = 0.0001,
    [double]$MinLr = 0.00007,
    [double]$MaxLr = 0.0002,
    [double]$StartK = 0.0,
...
if ([string]::IsNullOrWhiteSpace($RunName)) {
    $RunName = "win_g1_task2_3090_" + (Get-Date -Format "yyyyMMdd_HHmmss")
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host "G1 Task2 Windows RTX 3090 skrl PPO formal training runner"
Write-Host "============================================================"
...
if ($env:G1_TASK2_WINDOWS_TRAIN_APPROVED -ne "1") {
    Write-Host ""
    Write-Host "[STOP] To actually start Windows G1 Task2 training, run first:" -ForegroundColor Yellow
    Write-Host '  $env:G1_TASK2_WINDOWS_TRAIN_APPROVED = "1"'
    Write-Host ""
    Write-Host "Suggested first formal run:"
    Write-Host "  .\scripts\windows\train_task2_skrl_3090.ps1 -NumEnvs 128 -TotalEnvSteps 20000000"
    exit 0
}
...
elseif (-not [string]::IsNullOrWhiteSpace($Task1Pretrained)) {
...
Write-Host ""
Write-Host "============================================================"
Write-Host "G1 Task2 Windows RTX 3090 training finished with exit code: $exitCode"
